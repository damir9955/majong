...
export interface Battle {
   ...
   *  его счёт/прогресс приходят с сервера пуушами и поллингом. */
  online?: {
    code: string;
    playerId: string;
    seed: number;
    ...
    friendName?: string;
    friendHue?: number;
    /** счёт СЕРИИ побед с этим соперником (Task 32: растёт
     *  на сервере, живёт, пока в комнате одни и те же игроки) */
    myWins: number;
    friendWins: number;
...
/** Сколько рубашек можно держать открытыми одновременно.
 *  ОДНА (Фидбек Task 46: «почему одновременно открываются 2 закрытые
 *  карточки?»): переворот — либо мгновенная пара с ВИДИМЫМ соседом
 *  (вторая подгляданная или житель лотка), либо единственная
 *  открытая рубашка — новая закрывает предыдущую. */
export const PEEK_LIMIT = 1;
...
function removePair(
  ...
  // онлайн: мгновенная релея сопернику — «я собрал пару»
  if (s.mode === 'online' && battle) {
  ...
  persist(
    (set, get) => ({
      ...
      tapTile: (id) => {
        ...
        // ЗАКРЫТАЯ кость: тап — переворот; если её грань уже ВИДИМА
        // где-то (перевёрнутая ранее подглядка на доске ИЛИ житель
        // лотка — его грань всегда открыта) — пара собирается СРАЗУ,
        // вторым действием игрока (Фидбек Task 46: «при перевороте
        // такой же 2-й закрытой карточки она не улетает в лоток,
        // только после повторного нажатия»).
        //
        // Инварианты прежних правок сохранены:
        //  — улетает ТОЛЬКО тапнутая кость и её ВИДИМЫЙ сосед — никакая
        //    третья кость не стартует сама (Task 44/45: «нажимаешь на
        //    одну — другая сама улетает»);
        //  — авто-раскрытые рубашки (revealedIds) в спаривании НЕ
        //    участвуют — с ними пара по-прежнему через лоток (Task 39);
        //  — перевёрнутые игроком рубашки собираются с открытой
        //    веткой ниже (Task 43/1).
        // Подглядка всегда ОДНА: переворот новой рубашки закрывает
        // предыдущую — двух одновременно открытых не бывает (Фидбек
        // Task 46: «почему одновременно открываются 2 закрытые»).
        if (concealed) {
          // 1) ВИДИМАЯ пара на доске: ранее перевёрнутая игроком
          //    рубашка с той же гранью — «память»: перевернул обе —
          //    пара улетает сразу, без третьего тапа
          const peekTwin = findPeekTwin(s, matchKey, id);
          if (peekTwin !== null) {
            removePair(set, s, peekTwin, id);
            return;
          }
          // 2) ВИДИМАЯ пара в лотке: житель с той же гранью — кость
          //    разворачивается и летит к нему одним тапом
          ...
          // 3) пары нет — переворот; предыдущая подглядка гаснет
          playReveal();
          set({
            session: {
              ...s,
        ...
        // ПОДГЛЯДАННАЯ): улетает та, по которой тапнули, — и если
        // на доске лежит ВИДИМАЯ подгляданная рубашка с той же
        // гранью, обе летят парой (Task 43/1, Фидбек Task 45:
        // «закрытые карточки не улетают при нахождении такой же
        // не закрытой»). Подгляданная грань открыта игроку — это
        // не авто-сбор: игрок ВИДИТ, что соединяется.
        const peekTwin = findPeekTwin(s, matchKey, id);
        if (peekTwin !== null) {
          removePair(set, s, peekTwin, id);
          return;
      ...
      startOnlineSession: (view, playerId, opts) => {
        ...
        set({
          session: { ...base, battle },
          // уровень комнаты — и мой прогресс тоже (Фидбек Task 44:
          // «сыграли пять партий, а уровень всё девятый» — store-уровень
          // не синхронизировался, бейдж и новые приглашения стояли на месте)
          level: view.level,
          lastMode: 'online',
          showMenu: false,
        });
      ...
      applyRoomView: (view) => {
                ...
                battle: {
                  ...
                  online: b.online
                    ? {
                        ...
                        friendHue: friend?.hue,
                        myWins:
                          view.players.find((p) => p.id === creds.playerId)
                            ?.wins ?? b.online.myWins,
                        friendWins: friend?.wins ?? b.online.friendWins,
                      }
                    : b.online,
                },
              },
        ...
        if (
          ...
          const oppOff =
            view.advanceOffers?.find((o) => o.playerId !== creds.playerId)
              ?.kind ?? null;
          const meW = view.players.find((p) => p.id === creds.playerId)?.wins;
          const friend2 = view.players.find((p) => p.id !== creds.playerId);
          const myWins = meW ?? on.myWins;
          const friendWins = friend2?.wins ?? on.friendWins;
          if (
            on.myAdvance !== myOff ||
            on.oppAdvance !== oppOff ||
            on.myWins !== myWins ||
            on.friendWins !== friendWins
          ) {
            set({
              session: {
                ...cur,
