  ...
  persist(
    (set, get) => ({
      ...
      tapTile: (id) => {
        ...
        // подглядка): улетает ТОЛЬКО ТА, ПО КОТОРОЙ ТАПНУЛИ.
        // Перевёрнутая рубашка НЕ улетает сама (Фидбек Task 44:
        // «нажимаешь одну карточку — а вторая улетает автоматически»
        // — игра не должна играть за игрока). Пара соберётся, когда
        // игрок сам тапнет и её (путь выше: тап по закрытой, чья
        // пара в лотке, — одним тапом, без лишнего переворота).
        //
