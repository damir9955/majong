 ...
 *  — ячейки освобождаются СРАЗУ НА СТАРТЕ ВЗРЫВА (Task 44: скорость
 *    конвейера — «успеваю нажать три-четыре, а улетает одна»):
 *    пара уже съехалась в точку МЕЖДУ ячейками и растворяется там —
 *    новые плитки летят в ячейки, не дожидаясь конца взрыва;
 *  — когда взрыв закончился, остаток тихонько соскальзывает
 *    на освободившиеся ячейки.
 *
...
const RESERVE_MS = 80; // страховка после взрыва до финальной уборки

const FLIGHT_MS = 280; // обычный полёт доска → лоток одним скольжением
const TRAY_MS = 190; // спокойные сдвиги внутри лотка
const JOIN_MS = 170; // соединение пары в лотке
const BOOM_MS = 340; // взрыв пары
const FLIP_MS = 330; // переворот рубашки — полёт начинается ПОСЛЕ него
...
export function Board() {
   ...
   *  одиночки и соединяющиеся пары; ячейки ВЗРЫВАЮЩИХСЯ пар
   *  свободны С МОМЕНТА старта взрыва (Task 44: скорость) */
   ...
   * Ячейки освобождаются СРАЗУ НА СТАРТЕ ВЗРЫВА (Task 44): пара
   * уже съехалась в точку между ячейками и растворяется там — новые
   * плитки летят в ячейки, не дожидаясь конца взрыва; после конца —
   * компакт лотка.
   */
    ...
    (resident: number, arriving: number, waitMs: number, base: number) => {
      ...
      window.setTimeout(() => {
        ...
        window.setTimeout(() => {
          ...
          playBoom();
          buzz(20);
          // ЯЧЕЙКИ ОСВОБОЖДАЮТСЯ СРАЗУ НА СТАРТЕ ВЗРЫВА (Фидбек
          // Task 44: «успеваю нажать три-четыре — улетает одна»):
          // пара давно съехалась в точку МЕЖДУ ячейками и взрывается
          // там, растворяясь, — новые плитки летят в ячейки не
          // дожидаясь конца взрыва. Взрыв гаснет за BOOM_MS, а
          // новый полёт длится FLIGHT_MS — к прилёту старого взрыва
          // уже почти не видно. Сквозная скорость конвейера вместо
          // полной блокировки ряда на ~1,5 с.
          F.posOf.delete(resident);
          F.posOf.delete(arriving);
          // финальная уборка: гасим состояние пары и уплотняем остаток
          addFlyTimer(
            resident,
            window.setTimeout(() => {
  ...
  useEffect(() => {
    ...
    const unsub = useGame.subscribe((st, prev) => {
      ...
      // съезжаются, соединяются и взрываются; ячейки освобождаются
      // на СТАРТЕ взрыва — конвейер не останавливается (Task 44)
      //
